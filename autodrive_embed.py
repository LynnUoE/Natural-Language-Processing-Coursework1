...
def run(gdict,answer,answerFactory,errlog,grabPlots=False):
  ...
  gdict.update({'defaultdict':defaultdict})
  ...
  _run(gdict,answer,answerFactory,errlog,timeout=timeout)

def _exec(form,dict,timeout):
  if timeout>0:
    import stopit
    cc=False
...
def _run(gdict,answer,answerFactory,errlog,timeout=0):
  global counter
    ...
    try:
      for a in aLines:
        ...
        if (multi is not None) and (not mbs) and prefix.match(a):
          ...
          try:
            _exec(multi,gdict,timeout)
          except (TimeoutException,KeyboardInterrupt) as e:
            errs+=1
            print("""The following lines threw a %s:%s
  -------"""%(type(e),multi), file=errlog)
            multi=None
          except Exception as e:
            errs+=1
  ...
  -------"""%(e.__class__.__name__,bogus,a),
      ...
      try:
        ...
        if timeout>0:
          ...
          (ansd,uerrs)=answerFactory(gdict,errlog)
        errs+=uerrs
      except (Exception, KeyboardInterrupt, TimeoutException) as e:
